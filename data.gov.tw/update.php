    ...
    public function main()
    {
        if ($argv = $_SERVER['argv'][1]) {
            if (filter_var($argv, FILTER_VALIDATE_URL)) { // 網址就直接抓下來傳傳看
                $portal_meta = $this->getMetaFromPortal($argv);
                $config = array('source' => $argv);
                if ($_SERVER['argv'][2]) {
                    $config['meta_only'] = true;
                }
                $this->updateOrInsert(str_replace('/', '_', $portal_meta['title']), $config);
            } else {
                $this->updateOrInsert($argv, array());
            }
        } else {
            ...
            while ($url) {
                ...
                foreach ($sheets->data as $sheet_info) {
                    ...
                    $this->updateOrInsert($sheet_info->name, array('sheet_info' => $sheet_info));
                }
                $url = $sheets->next_url;
            }
        }
